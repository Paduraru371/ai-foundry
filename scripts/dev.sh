...
echo "    configuration loaded from $(pwd)/.env"
USES_AZURE=0
grep -qE '^(LLM_PROVIDER|EMBEDDING_PROVIDER)=azure$|^AGENT_MODE=foundry$' .env && USES_AZURE=1
USES_IDENTITY=0
grep -qE '^AZURE_AI_AUTH=identity$' .env && USES_IDENTITY=1

step 2 "Azure identity"
if [ "$USES_AZURE" = "1" ] && [ "$USES_IDENTITY" = "1" ]; then
  command -v az >/dev/null || {
    echo "    az not found: https://learn.microsoft.com/cli/azure/install-azure-cli"
    exit 1
  }
...
step 4 "Qdrant and the frontend"
...
ARGS=(backend.main:app --port "$PORT")
