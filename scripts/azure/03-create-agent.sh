...
FILE="$HERE/../../backend/agents/personas/$PERSONA.json"
[ -f "$FILE" ] || { echo "No persona '$PERSONA'. Available:"; ls "$HERE/../../backend/agents/personas" | sed 's/\.json$//;s/^/  /'; exit 2; }

# compose instructions exactly as backend/agents/persona.py does
