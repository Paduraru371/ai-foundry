...
"""Publish a persona to the Azure AI Foundry Agent Service.
    ...
    .venv/Scripts/python.exe scripts/deploy_agent.py lyrical

Reads backend/agents/personas/<name>.json, creates (or updates) an agent with the
...
from backend.agents.foundry_agent import FoundryUnavailable, deploy  # noqa: E402
from backend.agents.persona import PersonaNotFound, available_names, load_persona  # noqa: E402
...
def main() -> int:
    ...
    print(f"  · deploy all →  .venv/Scripts/python.exe scripts/deploy_agent.py --all")
