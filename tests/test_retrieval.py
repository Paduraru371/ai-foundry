...
from backend.api.routers.generation import ask
from backend.retrieval.pipeline import diversify, improve_retrieval, rerank
from backend.retrieval.reranker import parse_ranking, rerank_with_llm
from backend.schemas.agents import AskRequest
...
class RetrievalImprovementTests(unittest.TestCase):
    ...
    @patch("backend.api.routers.generation.retrieve", return_value=[])
    @patch("backend.api.routers.generation.store.info", return_value={"exists": True})
    @patch("backend.api.routers.generation.require_qdrant")
